...
class TimesheetBase(BaseModel):
    ...
    total_ot_hours_on_sundays: float = Field(..., description="Total overtime hours on Sundays", ge=0)
...
class TimesheetUpdate(BaseModel):
    ...
    total_sundays_worked: int | None = Field(None, description="Total Sundays worked", ge=0)
    total_ot_hours_on_sundays: float | None = Field(None, description="Total overtime hours on Sundays", ge=0)
    
    @model_validator(mode="after")
    def validate_sundays_vs_working_days(self) -> "TimesheetUpdate":
        """Validate that total Sundays worked is not more than total working days."""
        if (self.total_sundays_worked is not None and 
            self.total_working_days is not None and
            self.total_sundays_worked > self.total_working_days):
            raise ValueError("Total Sundays worked cannot exceed total working days")
        return self
