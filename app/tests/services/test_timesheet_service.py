...
async def test_get_employee_timesheets(mock_db_conn, mock_employee_uuid):
    ...
    with (
        ...
        ) as mock_get_employee_by_uuid,
        patch(
            "app.repositories.timesheet_repository.get_timesheets_by_employee"
        ) as mock_get_timesheets,
    ):
        # Set up mock return values
        mock_get_employee_by_uuid.return_value = {
            "uuid": str(mock_employee_uuid),
            "staff_code": "EMP001",
            "name": "John Doe",
        }
        ...
        result = await timesheet_service.get_employee_timesheets(
            mock_db_conn, mock_employee_uuid
        )

        # Verify result
        assert result == mock_timesheets
        mock_get_employee_by_uuid.assert_called_once_with(
            mock_db_conn, mock_employee_uuid
        )
...
@pytest.mark.asyncio
async def test_get_employee_timesheet_success(
    mock_db_conn, mock_employee_uuid, mock_timesheet_data
):
    ...
    with (
        ...
        patch(
            "app.repositories.timesheet_repository.get_timesheet"
        ) as mock_get_timesheet,
    ):
        # Set up mock return values
        mock_get_employee_by_uuid.return_value = {
            "uuid": str(mock_employee_uuid),
        ...
        result = await timesheet_service.get_employee_timesheet(
            mock_db_conn, mock_employee_uuid, year, month
        )
        ...
        mock_get_timesheet.assert_called_once_with(
            mock_db_conn, mock_employee_uuid, year, month
        )
...
async def test_get_employee_timesheet_not_found(mock_db_conn, mock_employee_uuid):
    ...
    with (
        ...
        with pytest.raises(HTTPException) as excinfo:
            await timesheet_service.get_employee_timesheet(
                mock_db_conn, mock_employee_uuid, year, month
            )
...
async def test_create_employee_timesheet_success(mock_db_conn, mock_employee_uuid):
    ...
    with (
        ...
        # Call the service function
        result = await timesheet_service.create_employee_timesheet(
            mock_db_conn, mock_employee_uuid, timesheet_data
        )
        ...
        mock_get_timesheet.assert_called_once_with(
            mock_db_conn, mock_employee_uuid, 2025, 6
        )
...
@pytest.mark.asyncio
async def test_create_employee_timesheet_duplicate(
    mock_db_conn, mock_employee_uuid, mock_timesheet_data
):
...
@pytest.mark.asyncio
async def test_update_employee_timesheet_success(
    mock_db_conn, mock_employee_uuid, mock_timesheet_data
):
    ...
    with (
        ...
        updated_timesheet = {
            ...
            "total_sundays_worked": mock_timesheet_data["total_sundays_worked"],
            "total_ot_hours_on_sundays": mock_timesheet_data[
                "total_ot_hours_on_sundays"
            ],
        }
        mock_update.return_value = updated_timesheet

        # Update data
        update_data = TimesheetUpdate(total_working_days=23, total_ot_hours=6.5)
...
@pytest.mark.asyncio
async def test_delete_employee_timesheet_success(
    mock_db_conn, mock_employee_uuid, mock_timesheet_data
):
