...
@pytest.mark.asyncio
async def test_create_timesheet_success(
    test_client, mock_employee_uuid, mock_timesheet_data, override_get_db
):
    """Test successful timesheet creation."""
    # Mock the create_employee_timesheet function
    with patch(
        "app.routers.timesheet_routes.timesheet_service.create_employee_timesheet"
    ) as mock_create:
        ...
        response = test_client.post(
            f"/employees/{mock_employee_uuid}/timesheets", json=mock_timesheet_data
        )
        ...
        assert (
            data["total_sundays_worked"] == mock_timesheet_data["total_sundays_worked"]
        )
        assert (
            data["total_ot_hours_on_sundays"]
            == mock_timesheet_data["total_ot_hours_on_sundays"]
        )


@pytest.mark.asyncio
async def test_create_timesheet_duplicate(
    test_client, mock_employee_uuid, mock_timesheet_data, override_get_db
):
    ...
    with patch(
        ...
        side_effect=HTTPException(
            status_code=409, detail="Timesheet for this month already exists"
        ),
    ):
        # Make the request
        response = test_client.post(
...
@pytest.mark.asyncio
async def test_create_timesheet_invalid_data(
    test_client, mock_employee_uuid, override_get_db
):
    ...
    # Make the request
    response = test_client.post(
        f"/employees/{mock_employee_uuid}/timesheets", json=invalid_data
    )
...
async def test_get_all_timesheets(test_client, mock_employee_uuid, override_get_db):
    ...
    with patch(
        "app.routers.timesheet_routes.timesheet_service.get_employee_timesheets"
    ) as mock_get_all:
...
@pytest.mark.asyncio
async def test_get_specific_timesheet(
    test_client, mock_employee_uuid, mock_timesheet_data, override_get_db
):
    """Test getting a specific timesheet by year and month."""
    # Mock the get_employee_timesheet function
    with patch(
        "app.routers.timesheet_routes.timesheet_service.get_employee_timesheet"
    ) as mock_get:
        ...
        response = test_client.get(
            f"/employees/{mock_employee_uuid}/timesheets/{year}/{month}"
        )
...
@pytest.mark.asyncio
async def test_get_timesheet_not_found(
    test_client, mock_employee_uuid, override_get_db
):
...
async def test_update_timesheet(test_client, mock_employee_uuid, override_get_db):
    ...
    with patch(
        "app.routers.timesheet_routes.timesheet_service.update_employee_timesheet"
    ) as mock_update:
        ...
        update_data = {"total_working_days": 23, "total_ot_hours": 6.5}
        response = test_client.put(
            f"/employees/{mock_employee_uuid}/timesheets/2025/6", json=update_data
        )
...
async def test_delete_timesheet(test_client, mock_employee_uuid, override_get_db):
    ...
    with patch(
        "app.routers.timesheet_routes.timesheet_service.delete_employee_timesheet"
    ) as mock_delete:
        # Make the request
