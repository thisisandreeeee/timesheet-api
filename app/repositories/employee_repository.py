...
async def get_employee_by_uuid(conn: aiosqlite.Connection, uuid: UUID) -> Optional[Dict]:
    """
    ...
    async with conn.execute(
        "SELECT uuid, staff_code, name FROM employees WHERE uuid = ?",
        (str(uuid),)
    ) as cursor:
...
async def get_employee_by_staff_code(conn: aiosqlite.Connection, staff_code: str) -> Optional[Dict]:
    """
...
async def update_employee(
    ...
    await conn.execute(
        f"UPDATE employees SET {set_clause} WHERE uuid = ?",
        values
    )
    await conn.commit()
...
async def delete_employee(conn: aiosqlite.Connection, uuid: UUID) -> bool:
    ...
    cursor = await conn.execute(
        "DELETE FROM employees WHERE uuid = ?",
        (str(uuid),)
    )
    await conn.commit()
